...
# print("Hello")
def process_data_loader(data, sum_data, start, end):
    ...
    for k in tqdm(range(start, end)):
        ...
        result_dict = {
            ...
            'force': torch.zeros((len(d), 3)), 
            'pbc': torch.tensor([1,1,1])  
        }

        atoms = convert_to_ase(result_dict)
        OptimTraj, OptimEnergies = FIRE_Relax(atoms, calculator, NStep=args.FIRE_steps, ftol=0.01, plot=False)
...
def main(args):
    ...
    results = process_data_loader(data, sum_data, args.start, args.end)
    sum_data.to_csv(args.out_path[:-4]+str(args.start)+"_"+str(args.end)+'.csv', index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--start", type=int, required=True, help="Start index of entries to process")
    parser.add_argument("--end", type=int, required=True, help="End index of entries to process")
