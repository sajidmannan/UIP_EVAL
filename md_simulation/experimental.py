...
def replicate_system(atoms, replicate_factors):
    ...
    original_positions = atoms.get_scaled_positions()@original_cell  #Scaled or Unscaled ?
...
def run_simulation(calculator, atoms, pressure=0.000101325, temperature=298, timestep=0.1, steps=10, SimDir='./'):  ##pressure=0.000101325 GPa
    ...
    dyn = NPTBerendsen(init_conf, timestep=timestep * units.fs, temperature_K=temperature,
                       pressure_au=pressure * units.bar, compressibility_au=4.57e-5 / units.bar)
    
    dyn.attach(MDLogger(dyn, init_conf, os.path.join(SimDir,"Simulation_thermo.log"), header=True, stress=True,
    ...
    lattice_parameters = []
    def write_frame():
        dyn.atoms.write(os.path.join(SimDir, f'MD_{atoms.get_chemical_formula()}_NPT.xyz'), append=True)
...
def main(args,config):
    ...
    if os.path.isdir(folder_path):
        for file in os.listdir(folder_path):
            ...
            avg_density, avg_angles, avg_lattice_parameters = run_simulation(calculator, atoms, pressure=Press, temperature=Temp, timestep=config.timestep, steps=config.runsteps, SimDir=SimDir,)
            print(avg_density)
            # Append the results to the data list
            data.append([file[:-4], density] + cell_lengths_and_angles + [avg_density] + avg_lattice_parameters.tolist() + avg_angles.tolist())
                # Create a DataFrame
