...
def run_simulation(atoms, runsteps=1000, SimDir='./'):
    ...
    _, initial_rdf = get_initial_rdf(atoms, perturb=20, noise_std=0.05, max_atoms=config.max_atoms, replicate=True)
    initial_bond_lengths, Initial_Pair_rdfs = get_bond_lengths_noise(atoms, perturb=20, noise_std=0.05, max_atoms=config.max_atoms, r_max=config.rdf_r_max)
    initial_temperature=config.temp
    # Replicate_system
    replication_factors, size = Symmetricize_replicate(len(atoms), max_atoms=config.max_atoms, box_lengths=atoms.get_cell_lengths_and_angles()[:3])
    ...
    #NPT
    MaxwellBoltzmannDistribution(atoms, temperature_K=config.temperature)
    dyn = NPTBerendsen(atoms, timestep=config.timestep * units.fs, temperature_K=config.temperature,
                       pressure_au=config.pressure * units.bar , compressibility_au=4.57e-5 / units.bar)
    
    dyn.attach(MDLogger(dyn, atoms, os.path.join(SimDir,"Simulation_thermo.log"), header=True, stress=True,
           peratom=False, mode="w"), interval=config.thermo_interval)
    ...
    dyn.attach(temperature_stability, interval=config.temperature_criteria_interval,
    ...
    def lindemann_stability(a=atoms):
        if len(traj) >= config.lindemann_traj_length:
            ...
            if lindemann_coefficient > config.max_linedmann_coefficient:
                logger.error(f"Unstable : Lindemann_coefficient={lindemann_coefficient:.6g} (> {config.max_linedmann_coefficient:.6g})")
                logger.error(f"Lindemann_stability criterion violated {lindemann_coefficient:.6g} > {config.max_linedmann_coefficient:.6g}, Stopping the simulation.")
                raise StabilityException()
            else:
                logger.info(f"Stable : Lindemann_coefficient={lindemann_coefficient:.6g} (< {config.max_linedmann_coefficient:.6g})")
    ...
    def rdf_stability(a=atoms):
        if len(traj) >= config.rdf_traj_length:
            ...
            if error_rdf > config.max_rdf_error_percent:
                logger.error(f"Unstable : RDF Error={error_rdf:.6g} (> {config.max_rdf_error_percent:.6g})")
                logger.error(f"RDF criterion violated. Stopping the simulation. WF={error_rdf:.6g}")
                raise StabilityException()
            else:
                logger.info(f"Stable : RDF Error={error_rdf:.6g} (< {config.max_rdf_error_percent:.6g})")
    ...
    def bond_lengths_stability(a=atoms):
        if len(traj) >= config.lindemann_traj_length:
            ...
            curr_bond_lengths, Pair_rdfs = get_bond_lengths_TrajAvg(traj[-config.rdf_traj_length:], r_max=config.rdf_r_max)
            for key in curr_bond_lengths.keys():
                ...
                if False: #error_percent > config.max_bond_error_percent:
                    logger.error(f"Unstable : Bond {key}={curr_bond_lengths[key]:.6g}, Initial={initial_bond_lengths[key]:.6g}, Error={error_percent:.6g} (> {config.max_bond_error_percent:.6g})")
                    ...
                    logger.info(f"Stable : Bond {key}: {error_percent: .6g} < {config.max_bond_error_percent:.6g} % Error")
...
def main (args, config):
    ...
    for batch in train_loader:
        if counter_batch==index:
            ...
            SimDir=os.path.join(config.out_dir, f'Simulation_{index}_{atoms.get_chemical_formula()}')
